...
export const SETTINGS_GROUPS: { title: string; fields: [string, string, string][] }[] = [
  ...
  },
  {
    title: "الإحالة",
    fields: [
      ["referral_prize_1", "جائزة المركز الأول (ر.س)", "تُدفع يدوياً من قِبلك لأفضل داعٍ كل سباق (٥٥ يوم)"],
...
export const FLAG_DESCRIPTIONS: Record<string, string> = {
  ...
  referral_program_enabled: "كل مستخدم يحصل على رابط دعوة خاص به — تُحسب له كل دعوة تكمّل تسجيلها، وأفضل ٣ داعين كل سباق (٥٥ يوم ثم استراحة ٥ أيام) يفوزون بجوائز نقدية (تُضبط قيمتها بالحقول أعلاه)",
